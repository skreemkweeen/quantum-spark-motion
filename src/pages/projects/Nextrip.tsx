import { motion, useScroll, useSpring, useTransform } from "framer-motion";import { useInView } from "react-intersection-observer";import { ReactNode, useEffect, useRef, useState } from "react";import { Link } from "react-router-dom";

import heroJet from "@/assets/nextrip/hero-jet-top.png";import phoneSearch from "@/assets/nextrip/phone-search.png";import splashTrio from "@/assets/nextrip/splash-trio.jpg";import phoneMercedes from "@/assets/nextrip/phone-mercedes.png";import bali from "@/assets/nextrip/bali.jpg";import clouds from "@/assets/nextrip/clouds.png";import phonesRow from "@/assets/nextrip/phones-row.jpg";import { NMark, NWordmark } from "@/components/nextrip/NMark";import { UserFlowDiagram } from "@/components/nextrip/UserFlowDiagram";

/* =========================================================================NEXTRIP — cinematic luxury aviation case study========================================================================= */

const ease = [0.16, 1, 0.3, 1] as const;

const fontDisplay = "'Cormorant Garamond', serif";const fontMono = "'DM Mono', ui-monospace, monospace";

const label: React.CSSProperties = {fontFamily: fontMono,textTransform: "uppercase",letterSpacing: "0.32em",fontSize: "10px",color: "rgba(240,237,232,0.45)",};

const labelXS: React.CSSProperties = {...label,fontSize: "8px",letterSpacing: "0.45em",color: "rgba(240,237,232,0.35)",};

/* ---------- Reveal ---------- */const Reveal = ({children,delay = 0,y = 28,className,}: { children: ReactNode; delay?: number; y?: number; className?: string }) => {const { ref, inView } = useInView({ threshold: 0.15, triggerOnce: true });return (<motion.divref={ref}initial={{ opacity: 0, y, filter: "blur(6px)" }}animate={inView ? { opacity: 1, y: 0, filter: "blur(0px)" } : {}}transition={{ duration: 1.1, delay, ease }}className={className}>{children}</motion.div>);};

/* ---------- Cursor ambient glow ---------- */const CursorGlow = () => {const ref = useRef<HTMLDivElement | null>(null);useEffect(() => {if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;if (window.matchMedia("(pointer: coarse)").matches) return;const el = ref.current;if (!el) return;let raf = 0, tx = innerWidth/2, ty = innerHeight/2, cx = tx, cy = ty;const tick = () => {cx += (tx - cx) * 0.08; cy += (ty - cy) * 0.08;el.style.transform = translate3d(${cx-360}px, ${cy-360}px, 0);raf = requestAnimationFrame(tick);};const onMove = (e: MouseEvent) => { tx = e.clientX; ty = e.clientY; };addEventListener("mousemove", onMove, { passive: true });raf = requestAnimationFrame(tick);return () => { removeEventListener("mousemove", onMove); cancelAnimationFrame(raf); };}, []);return (<divref={ref}aria-hiddenclassName="pointer-events-none fixed left-0 top-0 z-[5] h-[720px] w-[720px] rounded-full mix-blend-screen"style={{background: "radial-gradient(circle, rgba(220,228,240,0.10), rgba(180,195,220,0.05) 40%, transparent 70%)",filter: "blur(60px)",willChange: "transform",}}/>);};

/* ---------- Top scroll progress ---------- */const ScrollBar = () => {const { scrollYProgress } = useScroll();const sx = useSpring(scrollYProgress, { stiffness: 120, damping: 30, mass: 0.2 });return (<motion.divaria-hiddenclassName="pointer-events-none fixed left-0 right-0 top-0 z-[60] h-px origin-left"style={{scaleX: sx,background: "linear-gradient(90deg, transparent, rgba(220,228,240,0.95), transparent)",boxShadow: "0 0 12px rgba(220,228,240,0.6)",}}/>);};

/* ---------- Atmosphere layers (per section) ---------- */const Atmos = ({variant = "ambient",}: { variant?: "hero" | "ambient" | "wide" | "stage" }) => (

/* ---------- Bezel divider ---------- */const Bezel = () => (

/* ---------- Section index header ---------- */const SectionMark = ({ index, title }: { index: string; title: string }) => (

/* ---------- Glass card ---------- */const Glass = ({ children, className = "", style }: { children: ReactNode; className?: string; style?: React.CSSProperties }) => (

/* ---------- Device wrap with glow + floor pool ---------- */const Device = ({ children, className = "", float = true }: { children: ReactNode; className?: string; float?: boolean }) => (

/* ---------- Ghost wordmark ---------- */const Ghost = ({children,align = "center",className = "",}: { children: string; align?: "left" | "center" | "right"; className?: string }) => (

/* ---------- Hairline ---------- */const Hairline = ({ className = "" }: { className?: string }) => (

/* ---------- Section index rail (right) ---------- */const RAIL = [{ id: "hero", label: "Hero" },{ id: "overview", label: "Overview" },{ id: "identity", label: "Identity" },{ id: "splash", label: "Splash" },{ id: "concept", label: "Concept" },{ id: "userflow", label: "Userflow" },{ id: "tour", label: "Tour" },{ id: "profile", label: "Profile" },{ id: "showcase", label: "Showcase" },];const SectionRail = () => {const [active] = useStateActive();return ({RAIL.map((it, i) => {const on = i === active;return (<a href={#${it.id}} className="flex items-center gap-3" style={{ color: on ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.28)" }}><spanstyle={{ ...labelXS, color: "inherit", opacity: on ? 1 : 0, transform: on ? "translateX(0)" : "translateX(8px)", transition: "all .5s" }}>{it.label}<spanclassName="block h-px transition-all duration-500"style={{width: on ? 32 : 14,background: on ? "rgba(255,255,255,0.9)" : "rgba(255,255,255,0.25)",boxShadow: on ? "0 0 8px rgba(220,228,240,0.5)" : "none",}}/>);})});};function useStateActive(): [number] {const [a, s] = useState(0);useEffect(() => {const els = RAIL.map(i => document.getElementById(i.id)).filter(Boolean) as HTMLElement[];if (!els.length) return;const io = new IntersectionObserver((ents) => {ents.forEach(e => {if (e.isIntersecting) {const idx = els.indexOf(e.target as HTMLElement);if (idx >= 0) s(idx);}});}, { rootMargin: "-45% 0px -45% 0px" });els.forEach(el => io.observe(el));return () => io.disconnect();}, []);return [a];}

/* =========================================================================PAGE========================================================================= */

const Nextrip = () => {const { scrollYProgress } = useScroll();const heroY = useTransform(scrollYProgress, [0, 0.1], [0, -60]);

return (<divclassName="relative min-h-screen overflow-x-hidden text-white"style={{ background: "#040408" }}>{/* fixed atmospheric backdrop */}<divaria-hiddenclassName="pointer-events-none fixed inset-0 z-0"style={{background:"radial-gradient(ellipse 70% 50% at 50% 0%, rgba(40,55,90,0.22), transparent 65%), radial-gradient(ellipse 50% 40% at 50% 100%, rgba(30,40,70,0.18), transparent 70%)",}}/><divaria-hiddenclassName="pointer-events-none fixed inset-0 z-0 opacity-[0.05] mix-blend-overlay"style={{backgroundImage:"url("data/svg+xml;utf8,")",}}/>
